...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/dataverse_harvard_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"climate change"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/dataverse_harvard_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "dataverse-harvard": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-dataverse-harvard"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-dataverse-harvard
```
